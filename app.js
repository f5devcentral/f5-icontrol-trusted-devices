...
oas3Tools.initializeMiddleware(swaggerDoc, function (middleware) {
  ...
  app.use('/api-docs', serveStatic(path.join(__dirname, './api'), { 'index': ['swagger.json'] }));
  ...
  app.use('/docs/index.html', (req, res) => {
    res.write(swaggerUIIndex('/api-docs/swagger.json'));
    res.end();
  });
  // Serve up swagger-ui content
  app.use('/docs', serveStatic(pathToSwaggerUi, { 'index': ['index.html'] }));
